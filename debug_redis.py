    ...
    try:
        ...
        if keys:
            for key in keys[:5]:  # Show first 5 keys
                ...
                print(f"  - {key.decode()}: type={key_type}, ttl={ttl}s, size={size} bytes")
...
async def test_redis_storage(r, data, json_str):
    ...
    try:
        ...
        print(f"  Attempting to store {len(json_str) / (1024 * 1024):.2f} MB of data...")
        ...
        if "OOM" in str(e) or "maxmemory" in str(e):
            ...
            print("   2. Store only essential player data (filter out inactive/irrelevant players)")
...
async def test_compressed_storage(r, data):
    ...
    active_players = {k: v for k, v in data.items() 
